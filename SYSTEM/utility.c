...
#include "rs485.h"
/*******************************
名称：CheckCrc(u8* pbuff) & crc16one()
功能：CCITT校验，并不声明
参数：null
返回：null
*******************************/
u16  code  Crc16CCITT_Table[16] = { /* CRC 16bit余式表 */
...
u16 crc16_ccitt(u8 *ptr, u8 len) {
    ...
    crc = 0;
    while(len-- > 0) {
        da = ((u8)(crc >> 8)) >> 4;/* 暂存CRC的高四位 */
        crc <<= 4; /* CRC右移4位，相当于取CRC的低12位）*/
        crc ^= Crc16CCITT_Table[da ^ (*ptr / 16)]; /* CRC的高4位和本字节的前半字节相加后查表*/
        da = ((u8)(crc >> 8)) >> 4;/* 暂存CRC的高4位 */
        crc <<= 4; /* CRC右移4位， 相当于CRC的低12位） */
        crc ^= Crc16CCITT_Table[da ^ (*ptr & 0x0f)]; /* CRC的高4位和本字节的后半字节相加后查表*/
...
unsigned short crc16_ccitt(u8 *buf, int len)
{
